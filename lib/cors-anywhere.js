...
function parseURL(req_url) {
  ...
  var parsed = new url.URL(req_url);
  if (!parsed.hostname) {
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    var origin = req.headers.referer || req.headers.referred || req.headers.origin || '';
    if (origin){
    ...
    //add the referer to removeHeaders
    if (corsAnywhere.removeHeaders.indexOf('referer') < 0 ){
      corsAnywhere.removeHeaders.push('referer');
    }
